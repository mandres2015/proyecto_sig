...
io.on('connection', function (socket) {
  ...
  socket.on('sad', (data) => {
    oracle.connect().then(() => {
      try {
        const sql = `SELECT SUM(TOTAL) as total FROM FACTURA WHERE EXTRACT( YEAR FROM TO_DATE(fecha)) = '2019' AND sucursal = :1`
        var binds = [data.sucursal]
        oracle.execute(sql, binds, (err1, res1) => {
          var i = 0
          ...
          const objetivo = [10000, 7000, 5000]
          const totales = res1.rows
          const sql = `SELECT SUM(TOTAL), EXTRACT( MONTH FROM TO_DATE(fecha)) FROM FACTURA WHERE EXTRACT( YEAR FROM TO_DATE(fecha)) = '2019' AND SUCURSAL = :1 GROUP BY EXTRACT( MONTH FROM TO_DATE(fecha))`
          var binds = [data.sucursal]
          oracle.execute(sql, binds, (err2, res2) => {
            const totalMeses = res2.rows

            var values = []

            for (let i = 0; i < 12; i++) {
              var d = true
              totalMeses.forEach(element => {
                if (element[1] == i) {
            ...
            socket.emit('sad_data', {
              ...
              total: [Math.round(objetivo[i] * 100) / 100, Math.round(totales * 100) / 100]
            })
          })
        })
      } catch (Exception) {
